...
# load model output: L_I_T_Int_S_H_RT.RData

load("/path_to_model_output/L_I_T_Int_S_H_RT.RData")
...
lambda<-array(NA, dim=c(4, L_I_T_Int_S_H_RT$mcmc.info$n.samples))

#dry, dry =0, so no beta2.p
lambda[1,] <- exp(L_I_T_Int_S_H_RT$sims.list$beta1.lambda[,1])

lambda[2,] <- exp(L_I_T_Int_S_H_RT$sims.list$beta1.lambda[,2]) 

lambda[3,] <- exp(L_I_T_Int_S_H_RT$sims.list$beta1.lambda[,3])

lambda[4,] <- exp(L_I_T_Int_S_H_RT$sims.list$beta1.lambda[,4])
...
l.pred <- ggplot(lambda.prob, aes(Island, pred, ymin = lower, ymax = upper)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), breaks = seq(0, 50, by = 5), limits = c(0, 50)) +
...
theta<-array(NA, dim=c(2,8, L_I_T_Int_S_H_RT$mcmc.info$n.samples))

# dry season, beta2.theta= 

theta[1,1,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,1,1])

theta[1,2,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,2,1]) 

theta[1,3,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,3,1])

theta[1,4,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,4,1]) 

theta[1,5,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,5,1])

theta[1,6,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,6,1])

theta[1,7,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,7,1])

theta[1,8,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,8,1])

#wet, beta2.lamba =1
theta[2,1,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,1,2])

theta[2,2,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,2,2]) 

theta[2,3,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,3,2])

theta[2,4,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,4,2]) 

theta[2,5,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,5,2])

theta[2,6,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,6,2])

theta[2,7,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,7,2])

theta[2,8,] <- plogis(L_I_T_Int_S_H_RT$sims.list$beta1.theta[,8,2])
str(theta)
# then we take the mean of the mcmc list
...
theta.pred <- ggplot(data = theta.prob) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), breaks = seq(0, 1, by = 0.2), limits = c(0, 1)) +
