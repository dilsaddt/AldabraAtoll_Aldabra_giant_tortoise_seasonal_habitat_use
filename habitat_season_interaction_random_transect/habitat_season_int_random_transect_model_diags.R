...
# 
# lambda ~ Island
# theta ~ Season*Habitat+re(Transect)
# p~1
#
#
...
# prepare transect as site random effect for theta
transect<-c(transect_vec)
...
params <- c('beta1.lambda', "beta1.theta", 'p', 'randomtheta')
...
# Call Nimble, check convergence and summarize posteriors
start<-Sys.time()
L_I_T_Int_S_H_RT <- jags(bdata, 
                         inits, 
                         params, 
                         "L_I_T_Int_S_H_RT.txt", 
                         n.adapt = na, 
                         n.chains = nc,
                         n.thin = nt, 
                         n.iter = ni, 
...
save(L_I_T_Int_S_H_RT, file='L_I_T_Int_S_H_RT.RData')
...
L_I_T_Int_S_H_DIC <- L_I_T_Int_S_H_RT$DIC
L_I_T_Int_S_H_RT_DIC 

## 3.2. Convergence and distribution check ----
###############################################

# checking Rhat values, if there ara any over 1.1
hist(L_I_T_Int_S_H_RT$summary[,8])
length(which(L_I_T_Int_S_H_RT$summary[,8]>1.1))

# checking n.eff values, if there ara any under 100
length(which(L_I_T_Int_S_H_RT$summary[,9]<100))


# visual traceplot checking
MCMCtrace(object = L_I_T_Int_S_H_RT,
          pdf = TRUE, #export to PDF
          open_pdf = TRUE,
          filename='L_I_T_Int_S_H_RT_Trace',
...
plot_title <- ggtitle("Availability | Habitat-Season Interaction Model RT| Posterior distributions",
                      "medians and 95% intervals")

panel_background <- panel_bg(fill = 'white')

p<-mcmc_areas(L_I_T_Int_S_H_RT$samples,
...
plot_title <- ggtitle("Expected site-specific density | Habitat-Season Interaction Model RT| Posterior distributions",
                      "medians and 95% intervals")
p<-mcmc_areas(L_I_T_Int_S_H_RT$samples,
...
plot_title <- ggtitle("Random Transect Effect | Habitat-Season Interaction RT Model | Posterior distributions",
                      "medians and 95% intervals")

panel_background <- panel_bg(fill = 'white')

p<-mcmc_areas(L_I_T_Int_S_H_RT$samples,
              pars = c('randomtheta[1]','randomtheta[2]','randomtheta[3]','randomtheta[4]','randomtheta[5]',
                       'randomtheta[6]','randomtheta[7]','randomtheta[8]','randomtheta[9]','randomtheta[10]','randomtheta[11]','randomtheta[12]'),
              area_method = "equal height",
              prob = 0.95) + 
  plot_title + panel_background +
...
#checking summary
MCMCsummary(object = L_I_T_Int_S_H_RT, round = 3, params <- c('beta1.lambda',"beta1.theta",  "p",'randomtheta' ))[ ,c(1,3,5)]