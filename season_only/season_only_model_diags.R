  ...
  #
    ...
    for (i in 1:nsites){  
     ...
     N[i,t] ~ dbin(theta[i,t], M[i]) 
      logit(theta[i,t])<-beta1.theta[season[t]] # availability based on habitat (habitat use)
      for (j in 1:nsurveys){  
...
#load data file: 
#contains: y_with_0_effort_corrected.RData, Island.RData
...
save(L_I_T_S, file='L_I_T_S.RData')
...
hist(L_I_T_S$summary[,8])
length(which(L_I_T_S$summary[,8]>1.1))

# checking n.eff values, if there ara any under 100
length(which(L_I_T_S$summary[,9]<100))


# visual traceplot checking

MCMCtrace(object = L_I_T_S,
          pdf = TRUE, #export to PDF
          open_pdf = TRUE,
          filename='L_I_T_S_Trace',
...
plot_title <- ggtitle("Expected site-specific desnity | Season-only Model | Posterior distributions",
                      "medians and 95% intervals")

panel_background <- panel_bg(fill = 'white')

p<-mcmc_areas(L_I_T_S$samples,
...
plot_title <- ggtitle("Habitat Use | Season-only Model | Posterior distributions",
                      "medians and 95% intervals")
p<-mcmc_areas(L_I_T_S$samples,
...
p+scale_y_discrete(
  labels= c('beta1.theta[1]'='Dry',
            'beta1.theta[2]'='Wet'),
  limits= c('beta1.theta[2]',
            'beta1.theta[1]'))


#checking summary
MCMCsummary(object = L_I_T_S, round = 3, params <- c('beta1.lambda' ,"beta1.theta",  'p'))[ ,c(1,3,5)]
