...
# load model output: L_I_T_Add_S_H.RData

load("/path_to_model_output/L_I_T_Add_S_H.RData")
...
lambda<-array(NA, dim=c(4, L_I_T_Add_S_H$mcmc.info$n.samples))

#dry, dry =0, so no beta2.p
lambda[1,] <- exp(L_I_T_Add_S_H$sims.list$beta1.lambda[,1])

lambda[2,] <- exp(L_I_T_Add_S_H$sims.list$beta1.lambda[,2]) 

lambda[3,] <- exp(L_I_T_Add_S_H$sims.list$beta1.lambda[,3])

lambda[4,] <- exp(L_I_T_Add_S_H$sims.list$beta1.lambda[,4]) 
...
l.pred <- ggplot(lambda.prob, aes(Island, pred, ymin = lower, ymax = upper)) +
  ...
  ylab(expression(paste("Expected Abundance " (lambda)))) +
  xlab("Island") +
...
theta<-array(NA, dim=c(2,8, L_I_T_Add_S_H$mcmc.info$n.samples))

# dry season, beta2.theta= 0
theta[1,1,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,1]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,1])

theta[1,2,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,2]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,1]) 

theta[1,3,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,3]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,1])

theta[1,4,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,4]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,1]) 

theta[1,5,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,5]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,1])

theta[1,6,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,6]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,1])

theta[1,7,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,7]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,1]) 

theta[1,8,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,8]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,1]) 

#wet, beta2.lamba =1
theta[2,1,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,1]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,2])

theta[2,2,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,2]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,2])

theta[2,3,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,3]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,2])

theta[2,4,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,4]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,2])

theta[2,5,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,5]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,2])

theta[2,6,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,6]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,2])

theta[2,7,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,7]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,2])

theta[2,8,] <- exp(L_I_T_Add_S_H$sims.list$alpha.theta+
                     L_I_T_Add_S_H$sims.list$beta1.theta[,8]+
                     L_I_T_Add_S_H$sims.list$beta2.theta[,2])
...
ggsave("T_Pred.png", theta.pred, width = 12, height = 8, units = "in", dpi = 300, bg = "white")
