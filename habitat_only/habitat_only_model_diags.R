...
save(L_I_T_H, file='L_I_T_H.RData')
...
L_I_T_H_DIC <- L_I_T_H$DIC
L_I_T_H_DIC 

## 3.2. Convergence and distribution check ----
###############################################

# checking Rhat values, if there are any over 1.1
hist(L_I_T_H$summary[,8])
length(which(L_I_T_H$summary[,8]>1.1))

# checking n.eff values, if there are any under 100
length(which(L_I_T_H$summary[,9]<100))


# visual traceplot checking

MCMCtrace(object = L_I_T_H,
          pdf = TRUE, #export to PDF
          open_pdf = TRUE,
          filename='L_I_T_H_Trace',
          ind = TRUE, # separate density lines per chain
          Rhat = TRUE, # add Rhat
          n.eff = TRUE, # add eff sample size
          c('beta1.lambda', "beta1.theta", 'p'),
          iter=ni)

#caterpillar plots
color_scheme_set("teal")

plot_title <- ggtitle("Expected site-specific density | Habitat-only Model | Posterior distributions",
                      "medians and 95% intervals")

panel_background <- panel_bg(fill = 'white')

p<-mcmc_areas(L_I_T_H$samples,
              pars = c('beta1.lambda[1]',
                       'beta1.lambda[2]',
                       'beta1.lambda[3]',
                       'beta1.lambda[4]'),
...
plot_title <- ggtitle("Habitat Use | Habitat-only Model | Posterior distributions",
                      "medians and 95% intervals")

p<-mcmc_areas(L_I_T_H$samples,
              pars = c('beta1.theta[1]',
                       'beta1.theta[2]',
                       'beta1.theta[3]',
                       'beta1.theta[4]',
                       'beta1.theta[5]',
...
#checking summary
MCMCsummary(object = L_I_T_H, round = 3, params <- c('beta1.lambda' ,"beta1.theta",  'p'))[ ,c(1,3,5)]
