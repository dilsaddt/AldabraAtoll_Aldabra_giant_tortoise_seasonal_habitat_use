...
# load model output: L_I_T_H.RData

load("/path_to_model_output/L_I_T_H.RData")
...
lambda<-array(NA, dim=c(4, L_I_T_H$mcmc.info$n.samples))

#dry, dry =0, so no beta2.p
lambda[1,] <- exp(L_I_T_H$sims.list$beta1.lambda[,1])

lambda[2,] <- exp(L_I_T_H$sims.list$beta1.lambda[,2]) 

lambda[3,] <- exp(L_I_T_H$sims.list$beta1.lambda[,3])

lambda[4,] <- exp(L_I_T_H$sims.list$beta1.lambda[,4]) 
...
theta<-array(NA, dim=c(8, L_I_T_H$mcmc.info$n.samples))

# dry season, beta2.theta= 0
theta[1,] <- exp(L_I_T_H$sims.list$beta1.theta[,1])

theta[2,] <- exp(L_I_T_H$sims.list$beta1.theta[,2]) 

theta[3,] <- exp(L_I_T_H$sims.list$beta1.theta[,3])

theta[4,] <- exp(L_I_T_H$sims.list$beta1.theta[,4]) 

theta[5,] <- exp(L_I_T_H$sims.list$beta1.theta[,5])

theta[6,] <- exp(L_I_T_H$sims.list$beta1.theta[,6])

theta[7,] <- exp(L_I_T_H$sims.list$beta1.theta[,7]) 

theta[8,] <- exp(L_I_T_H$sims.list$beta1.theta[,8]) 
...
theta.prob <- expand.grid(Habitat=c('SMX','OMX','PEM','EXP','CAC','GRA','CHP','SND'))

theta.prob$pred[1:8] <- pm.theta[1:8]

theta.prob$lower[1:8] <- CRI.theta[1,1:8]

theta.prob$upper[1:8] <- CRI.theta[2,1:8]
...
#create plot
my_colors <- viridis::viridis(n=8, direction= -1)
...
theta.pred <- ggplot(data = theta.prob) +
  geom_crossbar(aes(Habitat, pred, col = Habitat, ymin = lower, ymax = upper), width = 0.3, position = position_dodge(width = 0.8)) +
  theme_minimal() +
  theme(
    panel.grid.minor = element_blank(),
    text = element_text(size = 20),
  ...
  labs(title = (expression(paste(theta,'~Habitat'))))
